...
def test_hub_devices_filter_single(tmp_hub):
    ...
    out = hub.devices(hub_id=tmp_hub.id, capabilities=hub.capability.COLOR_LOOP, devs=devs)
...
def test_hub_device_eligible(tmp_hub):
    ids, devs = tmp_hub.devices()
    assert hub.device_eligible(ids['lamp_osram'], hub.capability.COLOR_TEMP, devs=devs)
    assert not hub.device_eligible(ids['twilight_nexa'], hub.capability.COLOR_TEMP, devs=devs)
...
def test_hub_device_implicit_state(tmp_hub):
    ids, devs = tmp_hub.devices()
    state = {}
    hub.device_eligible(ids['lamp_osram'], hub.capability.COLOR_TEMP, devs=devs, state=state)
    assert 'temperature' in state
    hub.device_exists(ids['twilight_nexa'], devs=devs, state=state)
...
def test_hub_device_reachable(tmp_hub):
    ids, devs = tmp_hub.devices()
    assert hub.device_reachable(ids['reachable'], devs=devs)
    assert not hub.device_reachable(ids['not-reachable'], devs=devs)
...
def test_hub_device_exists(tmp_hub):
    ids, devs = tmp_hub.devices()
    assert hub.device_exists(ids['reachable'], devs=devs)
    assert not hub.device_exists('dead-beef', devs=devs)
